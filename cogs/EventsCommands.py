...
from utils.const import ACTIVITY, ACTIVITY_TYPE, JOIN_MESSAGE, STATUS_TYPE
...
class Events(commands.Cog):
    ...
    async def on_command_error(self, ctx, err):
        if isinstance(err, errors.MissingRequiredArgument) or isinstance(err, errors.BadArgument):
            ...
            await ctx.send("You've reached max capacity of command usage at once, "
    ...
    async def on_guild_join(self, guild):
        try:
            to_send = sorted([chan for chan in guild.channels if chan.permissions_for(
                guild.me).send_messages and isinstance(chan, discord.TextChannel)],
    ...
    async def on_member_join(self, member):
        ...
        return user_repo.add_or_update({
            "id": member.id,
            "name": member.name,
    ...
    async def on_member_remove(self, member):
        user_repo = UserRepository()
        return user_repo.delete({"id": member.id})
    ...
    async def on_ready(self):
        ...
        await self.bot.change_presence(
            activity=discord.Activity(
                type=activity_type.get(activity, 0), name=ACTIVITY
            ),
