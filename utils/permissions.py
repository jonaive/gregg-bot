...
async def check_priv(ctx, member):
    ...
    try:
        ...
        if ctx.author.top_role == member.top_role:
            return await ctx.send(f"You can't {ctx.command.name} someone \
                who has the same permissions as you...")
        if ctx.author.top_role < member.top_role:
            return await ctx.send(f"Nope, you can't {ctx.command.name} someone \
...
def can_handle(ctx, permission: str):
    ...
    return isinstance(ctx.channel, discord.DMChannel) \
        or getattr(ctx.channel.permissions_for(ctx.guild.me), permission)
